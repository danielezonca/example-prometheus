...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "example-prometheus-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name example-prometheus-service apps/example-prometheus-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app example-prometheus-service:1.0-SNAPSHOT
    oc expose svc/example-prometheus-service
else

	echo "Launching the application locally..."
	pattern="example-prometheus-service"
	files=( $pattern )
	cd ${files[0]}
